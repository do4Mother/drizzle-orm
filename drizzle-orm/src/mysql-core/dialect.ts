...
export class MySqlDialect {
  ...
  getTableNameNew(table: Table) {
    return getTableName(table).includes(".")
    ? getTableName(table).split(".").pop()?.replace(/`/g, "")
    : getTableName(table);
  } 
  ...
  buildSelectQuery({
    ...
    for (const f of fieldsList) {
      if (is(f.field, Column)) {
        ...
        if (
          this.getTableNameNew(f.field.table) !==
            (is(table, Subquery)
              ? table._.alias
          ...
          fieldsFlat: [
            ...
            ...((orderBy?.length ?? 0) > 0
              ? [
                  {
                    path: [],
                    field: sql`row_number() over (order by ${sql.join(
                      orderBy!,
                      sql`, `
                    )})`,
                  },
